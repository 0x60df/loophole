...
(ert-deftest loophole-test-local-variable-if-set-list ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (loophole--test-set-sample-condition)
    (let ((assumed (list 'loophole--map-alist
                                      'loophole--editing
                                      'loophole--timer-alist
                                      'loophole--editing-timer
                                      ...
                                      (intern "loophole-state")))
          (result (loophole-local-variable-if-set-list)))
      (should-not (seq-difference result assumed))
      (should-not (seq-difference assumed result)))))
                ...
                (let ((window-configuration
                  ...
                  (unless noninteractive
                    ...
                    (let ((display-buffer-alist
                           '((t display-buffer-pop-up-frame)))
                          (avoid-error (lambda (func &rest args)
                                         (if (frame-live-p (car args))
                                             (apply func args))))
                          (clean-advice (make-symbol
                                         "loophole-test-read-buffer")))
                      (unwind-protect
                          (progn
                            (advice-add 'set-frame-parameter
                                        :around avoid-error)
                            ;; Test if the function works
                            ;; even when frame is changed
                            ...
                            (let ((loophole-read-buffer-inhibit-recursive-edit
                                   t))
                              (condition-case nil
                                  (loophole-read-buffer
                                   (lambda (_)
...
(ert-deftest loophole-test-editing ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (with-temp-buffer
      (setq-default loophole--editing t)
      (setq loophole--editing (intern "loophole-test-a-map"))
...
(ert-deftest loophole-test-tag-string ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (should (equal (loophole-tag-string (intern "loophole"))
                   (string ?l ?o ?o ?p)))
    (loophole-tag (intern "loophole-test-a-map") nil)
    (should-not (loophole-tag-string (intern "loophole-test-a-map")))
    (loophole-tag (intern "loophole-2-map") (string ?9))
    (should (equal (loophole-tag-string (intern "loophole-2-map")) (string ?9)))
...
(ert-deftest loophole-test-toss-binding-form ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (let ((key (vector ?a))
      ...
      (loophole-bind-entry key (eval form))
      (should (member (cons key form)
                      (get (intern "loophole-3-map") :loophole-form-storage)))
      (let (advice-is-remained)
        (advice-mapc (lambda (advice _)
                       (if (and (symbolp advice)
                                (string-equal (symbol-name advice)
...
batch-mode, these assertions are skipped."
  ...
  (loophole--test-with-pseudo-environment
    ...
    (let ((function-cell (symbol-function (intern "loophole-22-map-state"))))
      (loophole--test-with-keyboard-events (string ?y ?e ?s ?\r)
    ...
    (let ((function-cell (symbol-function (intern "loophole-23-map-state"))))
      (loophole--test-with-keyboard-events (string ?n ?o ?\r)
    ...
    (let ((function-cell (symbol-function (intern "loophole-24-map-state"))))
...
(ert-deftest loophole-test-globalize ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (loophole-register (intern "loophole-1-map")
                       (intern "loophole-1-map-state"))
    (with-temp-buffer
      (set (intern "loophole-1-map-state") t)
      (let ((loophole-force-unintern t)
            (old-state-variable (intern "loophole-1-map-state")))
        (loophole-globalize (intern "loophole-1-map"))
        (should-not (assq old-state-variable loophole--map-alist))
        (should-not (assq old-state-variable
                          (default-value 'loophole--map-alist))))
      (should-not (local-variable-p (intern "loophole-1-map-state"))))
    (should (loophole-global-p (intern "loophole-1-map")))
    (set (intern "loophole-2-map") (make-sparse-keymap))
    (set (intern "loophole-2-map-state") nil)
    (make-variable-buffer-local (intern "loophole-2-map-state"))
    (loophole-register (intern "loophole-2-map")
                       (intern "loophole-2-map-state"))
    (let ((loophole--buffer-list nil))
      (with-temp-buffer
        (set (intern "loophole-2-map-state") t)
        (with-temp-buffer
          (set (intern "loophole-2-map-state") t)
          (let ((loophole-force-unintern t))
            (loophole-globalize (intern "loophole-2-map")))
          (should-not (memq (current-buffer) loophole--buffer-list))
          (should-not (memq #'loophole--follow-killing-local-variable
                            change-major-mode-hook))
          (should-not (memq #'loophole--follow-killing-local-variable
    ...
    (let* ((globalized nil)
           (map-variable nil)
           (loophole-after-globalize-functions
            (lambda (arg)
              (setq globalized (loophole-global-p arg))
              (setq map-variable arg)))
           (loophole-force-unintern t))
      (loophole-globalize (intern "loophole-3-map"))
      (should globalized)
      (should (eq map-variable (intern "loophole-3-map"))))
    (set (intern "loophole-4-map") (make-sparse-keymap))
    (set (intern "loophole-4-map-state") 1)
    (fset (intern "loophole-4-map-state") #'(lambda () 2))
    (put (intern "loophole-4-map-state") :loophole-test 3)
    (make-variable-buffer-local (intern "loophole-4-map-state"))
    (loophole-register (intern "loophole-4-map")
                       (intern "loophole-4-map-state"))
    (let ((function-cell (symbol-function (intern "loophole-4-map-state"))))
      (loophole--test-with-keyboard-events (string ?y ?e ?s ?\r)
        (loophole-globalize (intern "loophole-4-map")))
      (should-not (local-variable-if-set-p (intern "loophole-4-map-state")))
      (should (eql (symbol-value (intern "loophole-4-map-state")) 1))
      (should (equal (symbol-function (intern "loophole-4-map-state"))
                     function-cell))
      (should (eql (get (intern "loophole-4-map-state") :loophole-test) 3)))
    (set (intern "loophole-5-map") (make-sparse-keymap))
    (set (intern "loophole-5-map-state") 1)
    (fset (intern "loophole-5-map-state") #'(lambda () 2))
    (put (intern "loophole-5-map-state") :loophole-test 3)
    (make-variable-buffer-local (intern "loophole-5-map-state"))
    (loophole-register (intern "loophole-5-map")
                       (intern "loophole-5-map-state"))
    (let ((function-cell (symbol-function (intern "loophole-5-map-state")))
          (plist-cell (symbol-plist (intern "loophole-5-map-state"))))
      (loophole--test-with-keyboard-events (string ?n ?o ?\r)
        (should-error (loophole-globalize (intern "loophole-5-map"))
                      :type 'user-error))
      (should-not (loophole-global-p (intern "loophole-5-map")))
      (should (eql (symbol-value (intern "loophole-5-map-state")) 1))
      (should (equal (symbol-function (intern "loophole-5-map-state"))
                     function-cell))
      (should (equal (symbol-plist (intern "loophole-5-map-state"))
                     plist-cell)))
    (set (intern "loophole-6-map") (make-sparse-keymap))
    (set (intern "loophole-6-map-state") 1)
    (fset (intern "loophole-6-map-state") #'(lambda () 2))
    (put (intern "loophole-6-map-state") :loophole-test 3)
    (make-variable-buffer-local (intern "loophole-6-map-state"))
    (loophole-register (intern "loophole-6-map")
                       (intern "loophole-6-map-state"))
    (let ((function-cell (symbol-function (intern "loophole-6-map-state"))))
      (let ((loophole-force-unintern t))
        (loophole-globalize (intern "loophole-6-map")))
      (should (loophole-global-p (intern "loophole-6-map")))
      (should (eql (symbol-value (intern "loophole-6-map-state")) 1))
      (should (equal (symbol-function (intern "loophole-6-map-state"))
                     function-cell))
      (should (eql (get (intern "loophole-6-map-state") :loophole-test) 3)))
    (set (intern "loophole-7-map") (make-sparse-keymap))
    (set (intern "loophole-7-map-state") nil)
    (make-variable-buffer-local (intern "loophole-7-map-state"))
    (should-error (loophole-globalize (intern "loophole-7-map"))
                  :type 'user-error)
    (should (local-variable-if-set-p (intern "loophole-7-map-state")))
    (set (intern "loophole-8-map") (make-sparse-keymap))
    (set (intern "loophole-8-map-state") nil)
    (loophole-register (intern "loophole-8-map")
                       (intern "loophole-8-map-state")
                       nil t)
    (let* ((run nil)
           (loophole-after-globalize-functions (lambda (_) (setq run t))))
      (loophole-globalize (intern "loophole-8-map"))
      (should-not run))
    (should (loophole-global-p (intern "loophole-8-map")))
    (should-error (loophole-globalize nil) :type 'user-error)
    (should-error (loophole-globalize 0) :type 'wrong-type-argument)
    (should-error (loophole-globalize 1.0) :type 'wrong-type-argument)
...
(ert-deftest loophole-test-localize ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (loophole-register (intern "loophole-1-map")
                       (intern "loophole-1-map-state")
                       nil t)
    (loophole-localize (intern "loophole-1-map"))
    (should-not (loophole-global-p (intern "loophole-1-map")))
    (set (intern "loophole-2-map") (make-sparse-keymap))
    (set (intern "loophole-2-map-state") nil)
    (loophole-register (intern "loophole-2-map")
                       ...
                       nil t)
    (let ((loophole--buffer-list nil))
      (loophole-localize (intern "loophole-2-map"))
      (should (memq #'loophole--follow-adding-local-variable
                    (get-variable-watchers (intern "loophole-2-map-state")))))
    (set (intern "loophole-3-map") (make-sparse-keymap))
    (set (intern "loophole-3-map-state") nil)
    (loophole-register (intern "loophole-3-map")
                       (intern "loophole-3-map-state")
    ...
    (let* ((localized nil)
           (map-variable nil)
           (loophole-after-localize-functions
            (lambda (arg)
              (setq localized (not (loophole-global-p arg)))
              (setq map-variable arg))))
      (loophole-localize (intern "loophole-3-map"))
      (should localized)
      (should (eq map-variable (intern "loophole-3-map"))))
    (set (intern "loophole-4-map") (make-sparse-keymap))
    (set (intern "loophole-4-map-state") nil)
    ...
    (let ((loophole-force-make-variable-buffer-local nil))
      (loophole--test-with-keyboard-events (string ?y ?e ?s ?\r)
        (loophole-localize (intern "loophole-4-map"))))
    (should (local-variable-if-set-p (intern "loophole-4-map-state")))
    (set (intern "loophole-5-map") (make-sparse-keymap))
    (set (intern "loophole-5-map-state") 1)
    (loophole-register (intern "loophole-5-map")
                       (intern "loophole-5-map-state")
    ...
    (let ((loophole-force-make-variable-buffer-local nil))
      (loophole--test-with-keyboard-events (string ?n ?o ?\r)
        (should-error (loophole-localize (intern "loophole-5-map"))
                      :type 'user-error)))
    (should-not (local-variable-if-set-p (intern "loophole-5-map")))
    (set (intern "loophole-6-map") (make-sparse-keymap))
    (set (intern "loophole-6-map-state") nil)
    (should-error (loophole-localize (intern "loophole-6-map"))
                  :type 'user-error)
    (should-not (local-variable-if-set-p (intern "loophole-6-map-state")))
    (set (intern "loophole-7-map") (make-sparse-keymap))
    (set (intern "loophole-7-map-state") nil)
    (make-variable-buffer-local (intern "loophole-7-map-state"))
    (loophole-register (intern "loophole-7-map")
                       (intern "loophole-7-map-state"))
    (let* ((run nil)
           (loophole-after-localize-functions (lambda (_) (setq run t))))
      (loophole-localize (intern "loophole-7-map"))
      (should-not run))
    (should-not (loophole-global-p (intern "loophole-7-map")))
    (should-error (loophole-localize nil) :type 'user-error)
    (should-error (loophole-localize 0) :type 'wrong-type-argument)
    (should-error (loophole-localize 1.0) :type 'wrong-type-argument)
...
(ert-deftest loophole-test-enable ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (loophole-register (intern "loophole-3-map")
                       (intern "loophole-3-map-state"))
    (loophole-enable (intern "loophole-3-map"))
    (let* ((run nil)
           (loophole-after-enable-functions
            (lambda (_)
              (setq run t))))
      (loophole-enable (intern "loophole-3-map"))
      (should-not run)
      (should (symbol-value (intern "loophole-3-map-state"))))
    (set (intern "loophole-4-map") (make-sparse-keymap))
    (set (intern "loophole-4-map-state") nil)
    (should-error (loophole-enable (intern "loophole-4-map")) :type 'user-error)
...
(ert-deftest loophole-test-disable ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (with-temp-buffer
      ...
      (with-temp-buffer
        (loophole-enable (intern "loophole-1-map"))
        (loophole-disable (intern "loophole-1-map"))
        (should-not (symbol-value (intern "loophole-1-map-state"))))
      (should (symbol-value (intern "loophole-1-map-state")))
      (loophole-disable (intern "loophole-1-map"))
      (should-not (symbol-value (intern "loophole-1-map-state"))))
    ...
    (let* ((disabled nil)
           (map-variable nil)
           (loophole-after-disable-functions
            (lambda (arg)
              (setq disabled (not (symbol-value (loophole-state-variable arg))))
              (setq map-variable arg))))
      (loophole-disable (intern "loophole-2-map"))
      (should disabled)
      (should (eq map-variable (intern "loophole-2-map"))))
    (set (intern "loophole-3-map") (make-sparse-keymap))
    (set (intern "loophole-3-map-state") nil)
    ...
    (let* ((run nil)
           (loophole-after-disable-functions
            (lambda (_)
              (setq run t))))
      (loophole-disable (intern "loophole-3-map"))
      (should-not run)
      (should-not (symbol-value (intern "loophole-3-map-state"))))
    (set (intern "loophole-4-map") (make-sparse-keymap))
    (set (intern "loophole-4-map-state") nil)
    (should-error (loophole-disable (intern "loophole-4-map"))
                  :type 'user-error)
    (should-error (loophole-disable nil) :type 'user-error)
    (should-error (loophole-disable 0) :type 'wrong-type-argument)
    (should-error (loophole-disable 1.0) :type 'wrong-type-argument)
    (should-error (loophole-disable ?c) :type 'wrong-type-argument)
    (should-error (loophole-disable (cons 0 0)) :type 'wrong-type-argument)
    (should-error (loophole-disable (string ?s)) :type 'wrong-type-argument)
    (should-error (loophole-disable (vector ?v)) :type 'wrong-type-argument)
    (let ((return-args (lambda (&rest args) args)))
      (unwind-protect
          (progn
            (advice-add 'loophole-disable :override return-args)
            (set (intern "loophole-5-map") (make-sparse-keymap))
            (set (intern "loophole-5-map-state") t)
            (loophole-register (intern "loophole-5-map")
                               (intern "loophole-5-map-state"))
            (loophole--test-with-keyboard-events
                "loophole-5-map"
...
(ert-deftest loophole-test-disable-latest ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (with-temp-buffer
      ...
      (loophole-enable (intern "loophole-test-b-map"))
...
(ert-deftest loophole-test-localize-editing ()
  ...
  (loophole--test-with-pseudo-environment
    ...
    (with-temp-buffer
      ...
      (let* ((localized t)
             (map-variable nil)
             (loophole-after-localize-editing-functions
              (lambda (arg)
                (setq localized (not (loophole-global-editing-p)))
                (setq map-variable arg))))
        (loophole-localize-editing)
        (should localized)
        (should (eq map-variable (intern "loophole-test-a-map"))))
      (let* ((run nil)
             (loophole-after-localize-editing-functions
              (lambda (_) (setq run t))))
        (loophole-localize-editing)
...
(ert-deftest loophole-test-generate ()
  "Test for `loophole-generate'."
  (loophole--test-with-pseudo-environment
    (let ((loophole-force-unintern t))
      (should (eq (loophole-generate) (intern "loophole-1-map")))
      (should (eq (loophole-generate) (intern "loophole-2-map")))
      (should (eq (loophole-generate) (intern "loophole-3-map")))
      (should (eq (loophole-generate) (intern "loophole-4-map")))
      (should (eq (loophole-generate) (intern "loophole-5-map")))
      (should (eq (loophole-generate) (intern "loophole-6-map")))
      (should (eq (loophole-generate) (intern "loophole-7-map")))
      (should (eq (loophole-generate) (intern "loophole-8-map")))
      (should (eq (loophole-generate) (intern "loophole-1-map")))
      (loophole-unregister (intern "loophole-4-map"))
      (should (eq (loophole-generate) (intern "loophole-4-map")))
      (loophole-enable (intern "loophole-2-map"))
      ...
      (should (eq (loophole-generate) (intern "loophole-3-map")))
      (should (eq (loophole-generate) (intern "loophole-5-map")))
      (loophole-enable (intern "loophole-4-map"))
      (loophole-enable (intern "loophole-6-map"))
      (loophole-enable (intern "loophole-8-map"))
      (should (eq (loophole-generate) (intern "loophole-7-map")))
      (should (eq (loophole-generate) (intern "loophole-1-map")))
      (should (eq (loophole-generate) (intern "loophole-3-map")))
      (loophole-disable (intern "loophole-4-map"))
      (loophole-disable (intern "loophole-6-map"))
      (should (eq (loophole-generate) (intern "loophole-6-map")))
      (should (eq (loophole-generate) (intern "loophole-4-map")))
      (loophole-disable (intern "loophole-2-map"))
      (loophole-globalize (intern "loophole-2-map"))
      (should (eq (loophole-generate) (intern "loophole-2-map")))
      (should-not (loophole-global-p (intern "loophole-2-map")))
      (loophole-disable (intern "loophole-8-map"))
      (loophole-globalize (intern "loophole-8-map"))
      (loophole-unregister (intern "loophole-8-map"))
      (set (intern "loophole-8-map-state") t)
      (should (eq (loophole-generate) (intern "loophole-8-map")))
      (should-not (loophole-global-p (intern "loophole-8-map")))
      (should-not (default-value (intern "loophole-8-map-state")))
      (loophole-mode 1)
      (with-temp-buffer
        (loophole-enable (intern "loophole-1-map"))
        (loophole-enable (intern "loophole-2-map"))
        ...
        (loophole-enable (intern "loophole-4-map"))
        (loophole-enable (intern "loophole-6-map"))
        (loophole-enable (intern "loophole-7-map"))
        (loophole-enable (intern "loophole-8-map"))
        (with-temp-buffer
          (loophole-enable (intern "loophole-5-map"))
          (with-temp-buffer
            (should (eq (loophole-generate) (intern "loophole-5-map"))))
          (should-not (memq (current-buffer) loophole--buffer-list))
          (should-not (memq #'loophole--follow-killing-local-variable
          ...
          (should-not (memq #'loophole--follow-killing-local-variable
                            kill-buffer-hook)))
        (should-not (symbol-value (intern "loophole-5-map-state"))))
      (loophole-mode -1)
      (let ((loophole-temporary-map-max 4))
        (should (eq (loophole-generate) (intern "loophole-1-map")))
        (should (eq (loophole-generate) (intern "loophole-3-map")))
        (should (eq (loophole-generate) (intern "loophole-4-map")))
        (should (eq (loophole-generate) (intern "loophole-2-map")))
        (should (eq (loophole-generate) (intern "loophole-1-map"))))
      (let ((loophole-temporary-map-max 16))
        (should (eq (loophole-generate) (intern "loophole-10-map")))
        (should (eq (loophole-generate) (intern "loophole-11-map")))
        (should (eq (loophole-generate) (intern "loophole-12-map")))
        (should (eq (loophole-generate) (intern "loophole-13-map")))
        (should (eq (loophole-generate) (intern "loophole-14-map")))
        (should (eq (loophole-generate) (intern "loophole-15-map")))
        (should (eq (loophole-generate) (intern "loophole-16-map")))
        (should (eq (loophole-generate) (intern "loophole-9-map")))
        (should (eq (loophole-generate) (intern "loophole-7-map")))
        (should (eq (loophole-generate) (intern "loophole-6-map")))
        (should (eq (loophole-generate) (intern "loophole-8-map")))
        (should (eq (loophole-generate) (intern "loophole-5-map")))
