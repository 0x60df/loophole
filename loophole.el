...
returned."
  (or (eventp event) (signal 'wrong-type-argument (list 'eventp event)))
  (if (integerp event)
...
evaluated occasionally in future."
  (let ((advice (make-symbol "loophole-one-time-advice")))
    (fset advice (lambda (bind-entry binding-key binding-entry
                   ...
                   (unwind-protect
                       (let ((map-variable
                              (funcall bind-entry binding-key
                                       binding-entry binding-keymap)))
                         ...
                         (lambda (map-var)
                           (and (not
                                ...
                                (let ((i (string-to-number
                                          (match-string
                                           1 (symbol-name map-var)))))
                                  (and (< 0 i)
                                       (< i (1+ loophole-temporary-map-max))))))
...
Generated by `loophole-generate'." map-variable)))
    ...
    (dolist (buffer (loophole-buffer-list))
      (with-current-buffer buffer
        (when (local-variable-p state-variable)
          (kill-local-variable state-variable)
          (when (and (listp loophole--buffer-list)
                     (not (seq-some
                           #'local-variable-p
                           (loophole-local-variable-if-set-list))))
    ...
    (set-default state-variable nil)
